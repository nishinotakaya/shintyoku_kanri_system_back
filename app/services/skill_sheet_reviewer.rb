...
class SkillSheetReviewer
  SYSTEM = <<~SYS.freeze
    ...
    - 業務内容の改善版(suggestion)は「≪案件概要≫ / ≪担当業務≫ / ≪コメント≫」の3セクション構成にする (プロジェクト名は入れない)。各セクションは ≪≫ マーカーで始める。
      ...
      - ≪コメント≫: ここは端的にしすぎず、2〜4文で具体的に書く。取り組んだ工夫・課題への対処・チーム開発での姿勢(認識合わせ/上長やメンバーへの確認・相談の徹底/記録の共有)・関係者(運用担当・営業など)からの反応や評価・AI(Claude Code / Cursor)の活用方針 などを、読み手が仕事ぶりを具体的にイメージできるよう自然な文章で述べる。他セクションは端的にするが、コメントは内容の厚みを優先する。
