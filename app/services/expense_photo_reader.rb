...
# 実費レシート(ガソリン代など)の写真を OpenAI vision で読み取り、金額を返す。
# カレンダーの稼働報告書(運送)で「ガソリン代」を写真から自動入力するために使う。
# 期待出力:
#   - amount:     税込合計金額 (円・整数。読み取れなければ nil)
#   - label:      内容の短い表記 (例: "ガソリン代(ENEOS)")
...
class ExpensePhotoReader
  ...
  SYSTEM_PROMPT = <<~SYS.freeze
    ...
    主な対象はガソリンスタンドの給油レシートです(高速道路・駐車場などその他の実費レシートも読み取ります)。
    次の JSON で返してください:
    {
      "amount": 税込合計金額の整数 (円。読み取れなければ null),
      "label": 内容の短い表記 (例: "ガソリン代", "高速代(ETC)", "駐車場代"。店名がわかれば "ガソリン代(ENEOS)" のように付ける。20字以内),
      "confidence": 読み取りの確信度 0-100
    }
    【読み取りの目安】
